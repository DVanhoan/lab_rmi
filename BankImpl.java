...
public class BankImpl extends UnicastRemoteObject implements Interf {
    ...
    private Interf remotePeer; // server còn lại
    ...
    private void initDatabase() {
        try (Connection conn = DriverManager.getConnection("jdbc:sqlite:" + dbPath)) {
            ...
            st.executeUpdate(
                    "CREATE TABLE IF NOT EXISTS users(username TEXT PRIMARY KEY, password TEXT, balance REAL, logged_in INTEGER)");
            st.executeUpdate("INSERT OR IGNORE INTO users VALUES('alice','123',1000,0)");
            st.executeUpdate("INSERT OR IGNORE INTO users VALUES('bob','123',1500,0)");
        ...
        try (Connection conn = getConnection()) {
            ...
            if (rs.next()) {
                if (rs.getInt("logged_in") == 1)
                    return false; // đã đăng nhập ở nơi khác
